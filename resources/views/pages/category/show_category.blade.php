...
<div class="text-center pagination-container" >
    @if ($category_by_id->onFirstPage())
        ...
        ">Previous</span>
    @else
        <a href="{{ $category_by_id->previousPageUrl() }}" class="pagination-link" style="
    ...
    @for ($i = 1; $i <= $category_by_id->lastPage(); $i++)
        @if ($i == $category_by_id->currentPage())
            ...
            ">{{ $i }}</span>
        @else
            <a href="{{ $category_by_id->url($i) }}" class="pagination-link" style="
                border: 1px solid #007bff;
                border-radius: 5px;
                padding: 5px 10px;
                margin-right: 5px;
    ...
    @if ($category_by_id->hasMorePages())
        <a href="{{ $category_by_id->nextPageUrl() }}" class="pagination-link" style="
            border: 1px solid #007bff;
            border-radius: 5px;
            padding: 5px 10px;
            margin-left: 5px;
